...
{  // Data allocations are very expensive on CUDA. Use a buffer to solve:
   // allocate once reuse later.
...
static void help() {
  cout << "\n------------------------------------------------------------------"
          "--------"
       << endl
       << "This program shows how to port your CPU code to CUDA or write that "
          "from scratch."
       << endl
       << "You can see the performance improvement for the similarity check "
          "methods (PSNR and SSIM)."
       << endl
       << "Usage:" << endl
       << "./gpu-basics-similarity referenceImage comparedImage "
          "numberOfTimesToRunTest(like 10)."
       << endl
       << "--------------------------------------------------------------------"
          "------"
       << endl
...
int main(int, char* argv[]) {
  ...
  for (int i = 0; i < TIMES; ++i) result = getPSNR(I1, I2);

  time = 1000 * ((double)getTickCount() - time) / getTickFrequency();
  time /= TIMES;

  cout << "Time of PSNR CPU (averaged for " << TIMES << " runs): " << time
       << " milliseconds."
  ...
  for (int i = 0; i < TIMES; ++i) result = getPSNR_CUDA(I1, I2);

  time = 1000 * ((double)getTickCount() - time) / getTickFrequency();
  time /= TIMES;

  cout << "Time of PSNR CUDA (averaged for " << TIMES << " runs): " << time
       << " milliseconds."
       << " With result of: " << result << endl;
  ...
  time = 1000 * ((double)getTickCount() - time) / getTickFrequency();
  cout << "Initial call CUDA optimized:              " << time
       << " milliseconds."
       << " With result of: " << result << endl;
  ...
  for (int i = 0; i < TIMES; ++i) x = getMSSIM(I1, I2);

  time = 1000 * ((double)getTickCount() - time) / getTickFrequency();
  time /= TIMES;

  cout << "Time of MSSIM CPU (averaged for " << TIMES << " runs): " << time
       << " milliseconds."
       << " With result of B" << x.val[0] << " G" << x.val[1] << " R"
       << x.val[2] << endl;
  ...
  for (int i = 0; i < TIMES; ++i) x = getMSSIM_CUDA(I1, I2);

  time = 1000 * ((double)getTickCount() - time) / getTickFrequency();
  time /= TIMES;

  cout << "Time of MSSIM CUDA (averaged for " << TIMES << " runs): " << time
       << " milliseconds."
       << " With result of B" << x.val[0] << " G" << x.val[1] << " R"
       << x.val[2] << endl;
  ...
  time = 1000 * ((double)getTickCount() - time) / getTickFrequency();
  cout << "Time of MSSIM CUDA Initial Call            " << time
       << " milliseconds."
       << " With result of B" << x.val[0] << " G" << x.val[1] << " R"
       << x.val[2] << endl;
  ...
  cout << "Time of MSSIM CUDA OPTIMIZED ( / " << TIMES << " runs): " << time
       << " milliseconds."
       << " With result of B" << x.val[0] << " G" << x.val[1] << " R"
       << x.val[2] << endl
       << endl;
...
Scalar getMSSIM_CUDA(const Mat& i1, const Mat& i2) {
  ...
  Ptr<cuda::Filter> gauss =
      cuda::createGaussianFilter(vI2[0].type(), -1, Size(11, 11), 1.5);
  ...
  for (int i = 0; i < gI1.channels(); ++i) {
    ...
    cuda::addWeighted(mu1_2, 1.0, mu2_2, 1.0, C1,
                      t1);  // t1 = mu1_2 + mu2_2 + C1;
    cuda::addWeighted(sigma1_2, 1.0, sigma2_2, 1.0, C2,
                      t2);  // t2 = sigma1_2 + sigma2_2 + C2;
    cuda::multiply(
        t1, t2, t1);  // t1 =((mu1_2 + mu2_2 + C1).*(sigma1_2 + sigma2_2 + C2))
...
Scalar getMSSIM_CUDA_optimized(const Mat& i1, const Mat& i2, BufferMSSIM& b) {
  ...
  Ptr<cuda::Filter> gauss =
      cuda::createGaussianFilter(b.vI1[0].type(), -1, Size(11, 11), 1.5);
  ...
  for (int i = 0; i < b.gI1.channels(); ++i) {
    ...
    // operator*(Scalar, Mat)
    cuda::multiply(b.mu1_mu2, 2, b.t1, 1, -1,
                   stream);  // b.t1 = 2 * b.mu1_mu2 + C1;
    cuda::add(b.t1, C1, b.t1, cuda::GpuMat(), -1, stream);
    cuda::multiply(b.sigma12, 2, b.t2, 1, -1,
                   stream);  // b.t2 = 2 * b.sigma12 + C2;
    cuda::add(b.t2, C2, b.t2, cuda::GpuMat(), -12, stream);

    cuda::multiply(b.t1, b.t2, b.t3, 1, -1,
