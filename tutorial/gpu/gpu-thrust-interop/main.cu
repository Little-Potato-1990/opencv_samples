...
int main(void) {
  // Generate a 2 channel row matrix with 100 elements.  Set the first channel
  // to be the element index, and the second to be a randomly generated value.
  // Sort by the randomly generated value while maintaining index association.
  //! [sort]
  {
    ...
    // Fill the key channel with random numbers between 0 and 10.  A counting
    // iterator is used here to give an integer value for each location as an
    // input to prg::operator()
    thrust::transform(thrust::make_counting_iterator(0),
                      thrust::make_counting_iterator(d_data.cols), keyBegin,
                      prg(0, 10));
    // Sort the key channel and index channel such that the keys and indecies
    // stay together
  ...
  //! [random]
  {
    ...
    thrust::transform(thrust::make_counting_iterator(0),
                      thrust::make_counting_iterator(d_value.cols), valueBegin,
                      prg(-1, 1));
  ...
  //! [count_value]
  {
    ...
    auto count = thrust::count(GpuMatBeginItr<int>(d_value),
                               GpuMatEndItr<int>(d_value), 15);
    std::cout << count << std::endl;
  }
  //! [count_value]

  // Randomly fill an array then copy only values greater than 0.  Perform these
  // tasks on a stream.
  //! [copy_greater]
  {
    cv::cuda::GpuMat d_value(1, 100, CV_32F);
    ...
    //! [random_gen_stream]
    // Same as the random generation code from before except now the
    // transformation is being performed on a stream
    thrust::transform(thrust::system::cuda::par.on(
    ...
    int count =
        thrust::count_if(thrust::system::cuda::par.on(
                             cv::cuda::StreamAccessor::getStream(stream)),
                         valueBegin, valueEnd, pred_greater<float>(0.0));
    ...
    thrust::copy_if(thrust::system::cuda::par.on(
                        cv::cuda::StreamAccessor::getStream(stream)),
                    valueBegin, valueEnd, GpuMatBeginItr<float>(d_valueGreater),
                    pred_greater<float>(0.0));
