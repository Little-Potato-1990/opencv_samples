/*
 * gdal_image.cpp -- Load GIS data into OpenCV Containers using the Geospatial
 ...
 */
cv::Point2d lerp(cv::Point2d const& p1, cv::Point2d const& p2,
                 const double& t) {
...
/*
 * Given a pixel coordinate and the size of the input image, compute the pixel
 * location on the DEM image.
 */
...
int main(int argc, char* argv[]) {
  ...
  // load the image (note that we don't have the projection information.  You
  // will need to load that yourself or use the full GDAL driver.  The values
  // are pre-defined at the top of this file
  //![load1]
  ...
  //  Note:  This would be perfect for a configuration file, but is here for a
  //  working demo.
  color_range.push_back(
      std::pair<cv::Vec3b, double>(cv::Vec3b(188, 154, 46), -1));
  color_range.push_back(
      std::pair<cv::Vec3b, double>(cv::Vec3b(110, 220, 110), 0.25));
  color_range.push_back(
      std::pair<cv::Vec3b, double>(cv::Vec3b(150, 250, 230), 20));
  color_range.push_back(
      std::pair<cv::Vec3b, double>(cv::Vec3b(160, 220, 200), 75));
  color_range.push_back(
      std::pair<cv::Vec3b, double>(cv::Vec3b(220, 190, 170), 100));
  color_range.push_back(
      std::pair<cv::Vec3b, double>(cv::Vec3b(250, 180, 140), 200));
