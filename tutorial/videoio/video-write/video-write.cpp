...
int main(int argc, char *argv[]) {
  ...
  const bool askOutputType =
      argv[3][0] == 'Y';  // If false it will use the inputs codec type
  ...
  const string NAME = source.substr(0, pAt) + argv[2][0] +
                      ".avi";  // Form the new name with container
  int ex = static_cast<int>(
      inputVideo.get(CAP_PROP_FOURCC));  // Get Codec Type- Int form
  ...
  Size S =
      Size((int)inputVideo.get(CAP_PROP_FRAME_WIDTH),  // Acquire input size
