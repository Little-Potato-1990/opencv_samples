...
int main() {
  ...
  cout << "Color stream: " << colorStream.get(CAP_PROP_FRAME_WIDTH) << "x"
       << colorStream.get(CAP_PROP_FRAME_HEIGHT) << " @"
       << colorStream.get(CAP_PROP_FPS) << " fps" << endl;

  //! [Get properties]
  // Print depth stream parameters
  cout << "Depth stream: " << depthStream.get(CAP_PROP_FRAME_WIDTH) << "x"
       << depthStream.get(CAP_PROP_FRAME_HEIGHT) << " @"
    ...
    while (!isFinish) {
      ...
      if (depthStream.grab()) {
...

        {
          ...
          if (depthFrames.size() >= maxFrames) depthFrames.pop_front();
  ...
  std::thread colorReader([&] {
    while (!isFinish) {
      ...
      if (colorStream.grab()) {
        ...
        {
          std::lock_guard<std::mutex> lk(mtx);
          if (colorFrames.size() >= maxFrames) colorFrames.pop_front();
