 ...
 * @brief This program demonstrates how to use OpenCV PCA to extract the
...
void drawAxis(Mat &img, Point p, Point q, Scalar colour,
  ...
  //! [visualization1]
  double angle =
      atan2((double)p.y - q.y, (double)p.x - q.x);  // angle in radians
  double hypotenuse =
...
int main(int argc, char **argv) {
  ...
  CommandLineParser parser(argc, argv,
                           "{@input | pca_test1.jpg | input image}");
  parser.about(
      "This program demonstrates how to use OpenCV PCA to extract the "
      "orientation of an object.\n");
  parser.printMessage();
