...
static cv::Point3f get_nearest_3D_point(std::vector<cv::Point3f> &points_list,
  ...
  double d1 =
      std::sqrt(std::pow(p1.x - origin.x, 2) + std::pow(p1.y - origin.y, 2) +
                std::pow(p1.z - origin.z, 2));
  double d2 =
      std::sqrt(std::pow(p2.x - origin.x, 2) + std::pow(p2.y - origin.y, 2) +
                std::pow(p2.z - origin.z, 2));
...
void PnPProblem::set_P_matrix(const cv::Mat &R_matrix,
                              const cv::Mat &t_matrix) {
...
bool PnPProblem::estimatePose(const std::vector<cv::Point3f> &list_points3d,
  ...
  // Pose estimation
  bool correspondence =
      cv::solvePnP(list_points3d, list_points2d, A_matrix_, distCoeffs, rvec,
...
// Estimate the pose given a list of 2D/3D correspondences with RANSAC and the
// method to use

void PnPProblem::estimatePoseRANSAC(
    const std::vector<cv::Point3f>
        &list_points3d,  // list with model 3D coordinates
    const std::vector<cv::Point2f>
        &list_points2d,  // list with scene 2D coordinates
    ...
    float reprojectionError, double confidence)  // Ransac parameters
{
  ...
  cv::solvePnPRansac(list_points3d, list_points2d, A_matrix_, distCoeffs, rvec,
...
bool PnPProblem::backproject2DPoint(const Mesh *mesh,
