...
void drawQuestion(cv::Mat image, cv::Point3f point, cv::Scalar color) {
  ...
  cv::putText(image, text, cv::Point(25, 50), fontFace, fontScale, color,
              thickness_font, 8);
}
...
void drawText2(cv::Mat image, std::string text, cv::Scalar color) {
  cv::putText(image, text, cv::Point(25, 75), fontFace, fontScale, color,
              thickness_font, 8);
}
...
void drawFPS(cv::Mat image, double fps, cv::Scalar color) {
  ...
  cv::putText(image, fps_str, cv::Point(500, 50), fontFace, fontScale, color,
              thickness_font, 8);
}
...
void drawConfidence(cv::Mat image, double confidence, cv::Scalar color) {
  ...
  cv::putText(image, text, cv::Point(500, 75), fontFace, fontScale, color,
              thickness_font, 8);
}
...
void drawCounter(cv::Mat image, int n, int n_max, cv::Scalar color) {
  ...
  cv::putText(image, text, cv::Point(500, 50), fontFace, fontScale, color,
              thickness_font, 8);
}
...
void drawPoints(cv::Mat image, std::vector<cv::Point2f> &list_points_2d,
  ...
  for (unsigned int i = 0; i < list_points_2d.size(); ++i) {
    ...
    cv::putText(image, text, point_2d, fontFace, fontScale * 0.5, color,
                thickness_font, 8);
  }
}

// Draw only the 2D points
void draw2DPoints(cv::Mat image, std::vector<cv::Point2f> &list_points,
                  cv::Scalar color) {
...
void draw3DCoordinateAxes(cv::Mat image,
...
void drawObjectMesh(cv::Mat image, const Mesh *mesh, PnPProblem *pnpProblem,
                    cv::Scalar color) {
...
void createFeatures(const std::string &featureName, int numKeypoints,
  ...
  if (featureName == "ORB") {
    ...
    descriptor =
        cv::xfeatures2d::SURF::create(100, 4, 3, true);  // extended=true
#else
    std::cout
...
cv::Ptr<cv::DescriptorMatcher> createMatcher(const std::string &featureName,
                                             bool useFLANN) {
    ...
    if (useFLANN) {
      cv::Ptr<cv::flann::IndexParams> indexParams =
          cv::makePtr<cv::flann::LshIndexParams>(
              6, 12, 1);  // instantiate LSH index parameters
      cv::Ptr<cv::flann::SearchParams> searchParams =
          cv::makePtr<cv::flann::SearchParams>(
              50);  // instantiate flann search parameters
