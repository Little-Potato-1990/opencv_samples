...
int main(int argc, char* argv[]) {
  ...
  const String keys =
      ...
      "                   }"
      "{feature       |ORB   | feature name (ORB, KAZE, AKAZE, BRISK, SIFT, "
      "SURF, BINBOOST, VGG)  }";
  CommandLineParser parser(argc, argv, keys);

  string img_path = samples::findFile(
      "samples/cpp/tutorial_code/calib3d/real_time_pose_estimation/Data/"
      "resized_IMG_3875.JPG");  // image to register
  string ply_read_path = samples::findFile(
      "samples/cpp/tutorial_code/calib3d/real_time_pose_estimation/Data/"
      "box.ply");  // object mesh
  string write_path = samples::findFile(
      "samples/cpp/tutorial_code/calib3d/real_time_pose_estimation/Data/"
      "cookies_ORB.yml");  // output file
  ...
  if (parser.has("help")) {
    ...
    img_path = parser.get<string>("image").size() > 0
                   ? parser.get<string>("image")
                   : img_path;
    ply_read_path = parser.get<string>("mesh").size() > 0
                        ? parser.get<string>("mesh")
                        : ply_read_path;
    write_path = parser.get<string>("model").size() > 0
  ...
  while (waitKey(30) < 0) {
    ...
    // If the registration is not finished, draw which 3D point we have to
    // register. If the registration is finished, breaks the loop.
    if (!end_registration) {
      // Draw debug text
      int n_regist = registration.getNumRegist();
      int n_vertex = pts[n_regist];
      ...
      drawCounter(img_vis, registration.getNumRegist(),
                  registration.getNumMax(), red);
    } else {
      // Draw debug text
      drawText(img_vis, "END REGISTRATION", green);
      drawCounter(img_vis, registration.getNumRegist(),
                  registration.getNumMax(), green);
      break;
    }
  ...
  bool is_correspondence = pnp_registration.estimatePose(
      list_points3d, list_points2d, SOLVEPNP_ITERATIVE);
  ...
  // Check if keypoints are on the surface of the registration image and add to
  // the model
  for (unsigned int i = 0; i < keypoints_model.size(); ++i) {
    Point2f point2d(keypoints_model[i].pt);
    Point3f point3d;
    bool on_surface =
        pnp_registration.backproject2DPoint(&mesh, point2d, point3d);
    if (on_surface) {
