...
constexpr char kParserOptions[] =
    ...
    "{ landmpath    l ||      a path to a Landmarks detection model file "
    "(.xml).}"
    "{ landmdevice    |CPU|   the landmarks detection computation device.}"

    "{ input        i ||      a path to an input. Skip to capture from a "
    "camera.}"
    "{ boxes        b |false| set true to draw face Boxes in the \"Input\" "
    "window.}"
    "{ landmarks    m |false| set true to draw landMarks in the \"Input\" "
    "window.}"
...
inline int toIntRounded(const Tp x) {
  return static_cast<int>(std::lround(x));
}

//! [toDbl]
template <typename Tp>
inline double toDouble(const Tp x) {
...
struct Avg {
  struct Elapsed {
    explicit Elapsed(double ms) : ss(ms / 1000.), mm(toIntRounded(ss / 60)) {}
    const double ss;
    const int mm;
  };
  ...
  double tick() const {
    return std::chrono::duration_cast<MS>(now() - started).count();
  }
  Elapsed elapsed() const { return Elapsed{tick()}; }
  double fps(std::size_t n) const {
    return static_cast<double>(n) / (tick() / 1000.);
  }
};
...
std::string getWeightsPath(
    const std::string &mdlXMLPath)  // mdlXMLPath =
                                    // "The/Full/Path.xml"
{
...
namespace custom {
using TplPtsFaceElements_Jaw =
    std::tuple<cv::GArray<Landmarks>, cv::GArray<Contour>>;
...
G_TYPED_KERNEL(GFillPolyGContours, <cv::GMat(cv::GMat, cv::GArray<Contour>)>,
    ...
    static cv::GMatDesc outMeta(cv::GMatDesc in,
                                cv::GArrayDesc){return in.withType(CV_8U, 1);
}
}
;
...
G_TYPED_KERNEL(GPolyLines,
    ...
    static cv::GMatDesc outMeta(cv::GMatDesc in, cv::GArrayDesc, bool,
                                cv::Scalar){return in;
}
}
;
...
G_TYPED_KERNEL(GFacePostProc, <GArrayROI(cv::GMat, cv::GMat, float)>,
    ...
    static cv::GArrayDesc outMeta(const cv::GMatDesc &, const cv::GMatDesc &,
                                  float){return cv::empty_array_desc();
}
}
;
...
//! [kern_m_decl]
using TplFaces_FaceElements =
    std::tuple<cv::GArray<Contour>, cv::GArray<Contour>>;
G_TYPED_KERNEL_M(
    GGetContours,
...
GAPI_OCV_KERNEL(GCPULaplacian, custom::GLaplacian){
    static void run(const cv::Mat &src, const int ddepth,
                    cv::Mat &out){cv::Laplacian(src, out, ddepth);
}
}
;
...
GAPI_OCV_KERNEL(GCPUPolyLines, custom::GPolyLines){
    static void run(const cv::Mat &src, const std::vector<Contour> &cnts,
                    const bool isClosed, const cv::Scalar &color,
...
GAPI_OCV_KERNEL(GCPURectangle, custom::GRectangle){
    static void run(const cv::Mat &src, const VectorROI &vctFaceBoxes,
                    const cv::Scalar &color, cv::Mat &out){src.copyTo(out);
...
GAPI_OCV_KERNEL(GCPUFacePostProc, GFacePostProc){
    static void run(const cv::Mat &inDetectResult, const cv::Mat &inFrame,
                    ...
                    VectorROI &outFaces){const int kObjectSize = 7;
...
GAPI_OCV_KERNEL(GCPULandmPostProc, GLandmPostProc){static void run(
    const std::vector<cv::Mat> &vctDetectResults, const VectorROI &vctRects,
...
//! [ld_pp_cnts]
//! [kern_m_impl]
GAPI_OCV_KERNEL(GCPUGetContours, GGetContours){static void run(
    const std::vector<Landmarks>
        &vctPtsFaceElems,  // 18 landmarks of the facial elements
...
//  in an input image, namely 4 elements (two eyes, nose, mouth) for every
//  detected face:
vctElemsContours.reserve(numFaces * 4);
// vctFaceElemsContours will store all the faces' contours found in an input
// image:
...
for (size_t i = 0ul; i < numFaces; i++) {
  ...
  // A left eye:
  // Approximating the lower eye contour by half-ellipse (using eye points) and
  // storing in cntLeftEye:
  cntLeftEye = getEyeEllipse(vctPtsFaceElems[i][1], vctPtsFaceElems[i][0]);
  // Pushing the left eyebrow clock-wise:
  cntLeftEye.insert(
      cntLeftEye.end(),
      {vctPtsFaceElems[i][12], vctPtsFaceElems[i][13], vctPtsFaceElems[i][14]});

  // A right eye:
  // Approximating the lower eye contour by half-ellipse (using eye points) and
  // storing in vctRightEye:
  cntRightEye = getEyeEllipse(vctPtsFaceElems[i][2], vctPtsFaceElems[i][3]);
  // Pushing the right eyebrow clock-wise:
  cntRightEye.insert(
      cntRightEye.end(),
      {vctPtsFaceElems[i][15], vctPtsFaceElems[i][16], vctPtsFaceElems[i][17]});
  ...
  // A mouth:
  // Approximating the mouth contour by two half-ellipses (using mouth points)
  // and storing in vctMouth:
  ...
  vctElemsContours.insert(vctElemsContours.end(),
                          {cntLeftEye, cntRightEye, cntNose, cntMouth});

  // The face contour:
  // Approximating the forehead contour by half-ellipse (using jaw points) and
  // storing in vctFace:
  cntFace =
      getForeheadEllipse(vctCntJaw[i][0], vctCntJaw[i][16], vctCntJaw[i][8]);
  // The ellipse is drawn clock-wise, but jaw contour points goes vice versa, so
  // it's necessary to push
...
int main(int argc, char **argv) {
  ...
  cv::GComputation pipeline([=]() {
    ...
    //! [net_usg_fd]
    GArrayROI garRects = custom::GFacePostProc::on(
        faceOut, gimgIn, config::kConfThresh);  // post-proc

    //! [net_usg_ld]
    cv::GArray<cv::GMat> landmOut =
        cv::gapi::infer<custom::LandmDetector>(garRects, gimgIn);
    //! [net_usg_ld]
    cv::GArray<Landmarks> garElems;  // |
    cv::GArray<Contour> garJaws;     // |output arrays
    std::tie(garElems, garJaws) =
        custom::GLandmPostProc::on(landmOut, garRects);  // post-proc
    cv::GArray<Contour> garElsConts;                     // face elements
    cv::GArray<Contour> garFaceConts;                    // whole faces
    std::tie(garElsConts, garFaceConts) =
        custom::GGetContours::on(garElems, garJaws);  // interpolation

    //! [msk_ppline]
    cv::GMat mskSharp =
        custom::GFillPolyGContours::on(gimgIn, garElsConts);  // |
    cv::GMat mskSharpG =
        cv::gapi::gaussianBlur(mskSharp, config::kGKernelSize,  // |
                               config::kGSigma);                // |
    cv::GMat mskBlur =
        custom::GFillPolyGContours::on(gimgIn, garFaceConts);  // |
    cv::GMat mskBlurG =
        cv::gapi::gaussianBlur(mskBlur, config::kGKernelSize,  // |
                               config::kGSigma);               // |draw masks
    // The first argument in mask() is Blur as we want to subtract from // |
    // BlurG the next step: // |
    cv::GMat mskBlurFinal =
        mskBlurG - cv::gapi::mask(mskBlurG, mskSharpG);   // |
    ...
    cv::GMat gimgBilat = custom::GBilatFilter::on(
        gimgIn, config::kBSize, config::kBSigmaCol, config::kBSigmaSp);
    cv::GMat gimgSharp =
        custom::unsharpMask(gimgIn, config::kUnshSigma, config::kUnshStrength);
    // Applying the masks
    ...
    return cv::GComputation(cv::GIn(gimgIn),
                            cv::GOut(gimgBeautif, cv::gapi::copy(gimgIn),
                                     garFaceConts, garElsConts, garRects));
  });
  //! [ppl]
  // Declaring IE params for networks
  //! [net_param]
  auto faceParams =
      cv::gapi::ie::Params<custom::FaceDetector>{/*std::string*/ faceXmlPath,
                                                 /*std::string*/ faceBinPath,
                                                 /*std::string*/ faceDevice};
  auto landmParams =
      cv::gapi::ie::Params<custom::LandmDetector>{/*std::string*/ landmXmlPath,
  ...
  //! [kern_pass_1]
  auto customKernels =
      cv::gapi::kernels<custom::GCPUBilateralFilter, custom::GCPULaplacian,
                        custom::GCPUFillPolyGContours, custom::GCPUPolyLines,
                        custom::GCPURectangle, custom::GCPUFacePostProc,
                        custom::GCPULandmPostProc, custom::GCPUGetContours>();
  auto kernels =
      cv::gapi::combine(cv::gapi::core::fluid::kernels(), customKernels);
  //! [kern_pass_1]
  ...
  if (flgStreaming == true) {
    ...
    cv::GStreamingCompiled stream =
        pipeline.compileStreaming(cv::compile_args(kernels, networks));
    //! [str_comp]
    ...
    //! [str_src]
      ...
      stream.setSource(
          cv::gapi::wip::make_src<cv::gapi::wip::GCaptureSource>(0));
    }
...
#include <opencv2/videoio.hpp>
    ...
    if (flgPerformance == true) {
      while (true) {
        ...
        frames++;
        if (!cc) {
          cc = pipeline.compile(cv::descr_of(img),
