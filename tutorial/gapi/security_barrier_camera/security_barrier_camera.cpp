...
struct Avg {
  struct Elapsed {
    explicit Elapsed(double ms)
        : ss(ms / 1000.), mm(static_cast<int>(ss) / 60) {}
    const double ss;
    const int mm;
  };
...
namespace custom {
G_API_NET(VehicleLicenseDetector, <cv::GMat(cv::GMat)>,
...
G_API_OP_M(ProcessDetections, <GVehiclesPlates(cv::GMat, cv::GMat)>,
...
GAPI_OCV_KERNEL(OCVProcessDetections, ProcessDetections){
    static void run(const cv::Mat &in_ssd_result, const cv::Mat &in_frame,
...
for (int i = 0; i < MAX_PROPOSALS; i++) {
  ...
  switch (static_cast<int>(label)) {
    case 1:
      out_vehicles.push_back(rc & surface);
      break;
    case 2:
      out_plates.emplace_back((rc - PT(15, 15) + SZ(30, 30)) & surface);
      break;
    default:
      CV_Assert(false && "Unknown object class");
  }
}
}
}
...
namespace labels {
const std::string colors[] = {"white", "gray", "yellow", "red",
                              "green", "blue", "black"};
const std::string types[] = {"car", "van", "truck", "bus"};
const std::vector<std::string> license_text = {"0",
                                               "1",
                                               "2",
                                               "3",
                                               "4",
                                               ...
                                               "Z"};
namespace {
void DrawResults(cv::Mat &frame, const std::vector<cv::Rect> &vehicles,
  ...
  for (auto it = vehicles.begin(); it != vehicles.end(); ++it) {
    ...
    const auto color_id =
        std::max_element(colors_data, colors_data + 7) - colors_data;
    const auto type_id =
        std::max_element(types_data, types_data + 4) - types_data;
  ...
  for (auto it = plates.begin(); it != plates.end(); ++it) {
    ...
    cv::putText(frame, result, cv::Point(rc.x, y_pos),
...
void DrawFPS(cv::Mat &frame, std::size_t n, double fps) {
  ...
  out << "FRAME " << n << ": " << std::fixed << std::setprecision(2) << fps
      << " FPS (AVG)";
  cv::putText(frame, out.str(), cv::Point(0, frame.rows),
...
int main(int argc, char *argv[]) {
  ...
  cv::GComputation pp([]() {
    ...
    std::tie(colors, types) =
        cv::gapi::infer<custom::VehicleAttributes>(vehicles, in);
    ...
    return cv::GComputation(
        cv::GIn(in), cv::GOut(frame, vehicles, colors, types, plates, numbers));
  });
  ...
  auto attr_net =
      cv::gapi::ie::Params<custom::VehicleAttributes>{
  ...
  if (cmd.get<bool>("ser")) {
    ...
    while (cv::waitKey(1) < 0) {
      ...
      frames++;
      labels::DrawResults(frame, vehicles, out_colors, out_types, plates,
                          out_numbers);
    ...
    while (cc.running()) {
      auto out_vector =
          cv::gout(frame, vehicles, out_colors, out_types, plates, out_numbers);
      if (no_show) {
        // This is purely a video processing. No need to balance with UI
        // rendering. Use a blocking pull() to obtain data. Break the loop if
        // the stream is over.
        if (!cc.pull(std::move(out_vector))) break;
      ...
      // At this point we have data for sure (obtained in either blocking or
      // non-blocking way).
      frames++;
      labels::DrawResults(frame, vehicles, out_colors, out_types, plates,
                          out_numbers);
