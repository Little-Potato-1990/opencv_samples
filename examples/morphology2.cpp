...
static void OpenClose(int, void*) {
  ...
  Mat element = getStructuringElement(
      element_shape, Size(an * 2 + 1, an * 2 + 1), Point(an, an));
  if (n < 0)
...
int main(int argc, char** argv) {
  ...
  createTrackbar("iterations", "Open/Close", &open_close_pos, max_iters * 2 + 1,
                 OpenClose);
  ...
  createTrackbar("iterations", "Erode/Dilate", &erode_dilate_pos,
