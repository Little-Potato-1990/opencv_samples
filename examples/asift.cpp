...
static void help(char** argv) {
  cout << "This is a sample usage of AffineFeature detector/extractor.\n"
       ...
       << "     [ --flann ]                            # use Flann-based "
          "matcher instead of bruteforce.\n"
       << "     [ --maxlines=<number(50 as default)> ] # The maximum number of "
          "lines in visualizing the matching result.\n"
       << "     [ --image1=<image1(aero1.jpg as default)> ]\n"
       << "     [ --image2=<image2(aero3.jpg as default)> ] # Path to images "
          "to compare."
       << endl;
}

static double timer() { return getTickCount() / getTickFrequency(); }
...
int main(int argc, char** argv) {
  ...
  if (!parser.check()) {
    parser.printErrors();
    cout << "See --help (or missing '=' between argument name and value?)"
         << endl;
    return 1;
  }
  ...
  if (feature == "sift") {
    ...
    if (useFlann)
      matcher =
          makePtr<FlannBasedMatcher>(makePtr<flann::LshIndexParams>(6, 12, 1));
    else
  ...
  cout << "img1 - " << kp1.size() << " features, "
       << "img2 - " << kp2.size() << " features" << endl;

  cout << "matching with " << (useFlann ? "flann" : "bruteforce") << "..."
       << endl;
  double start = timer();
  // match and draw
