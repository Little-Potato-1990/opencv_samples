...
static int print_help(char** argv) {
  cout << " Given a list of chessboard images, the number of corners (nx, ny)\n"
       ...
       << endl;
  cout << "Usage:\n " << argv[0]
       << " -w=<board_width default=9> -h=<board_height default=6> "
          "-s=<square_size default=1.0> <image list XML/YML file "
...
static void StereoCalib(const vector<string>& imagelist, Size boardSize,
                        float squareSize, bool displayCorners = false,
  ...
  cameraMatrix[0] =
      initCameraMatrix2D(objectPoints, imagePoints[0], imageSize, 0);
  cameraMatrix[1] =
      initCameraMatrix2D(objectPoints, imagePoints[1], imageSize, 0);
  Mat R, T, E, F;
...
int main(int argc, char** argv) {
  ...
  if (parser.has("help")) return print_help(argv);
