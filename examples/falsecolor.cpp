...
static const String ColorMaps[] = {"Autumn",   "Bone",
                                   "Jet",      "Winter",
                                   "Rainbow",  "Ocean",
                                   "Summer",   "Spring",
                                   "Cool",     "HSV",
...
static void TrackColorMap(int x, void *r) {
  ...
  putText(dst, "Colormap : " + ColorMaps[p->iColormap], Point(10, 20),
...
static Mat DrawMyImage(int thickness, int nbShape) {
  ...
  for (int i = 0; i < 256; i++)
    line(img, Point(thickness * i + offsetx, offsety),
         Point(thickness * i + offsetx, offsety + lineLength), Scalar(i),
         thickness);
  RNG r;
  Point center;
  int radius;
  ...
  for (int i = 1; i <= nbShape; i++) {
    ...
    switch (typeShape) {
      case MyCIRCLE:
        center = Point(r.uniform(offsetx, img.cols - offsetx),
                       r.uniform(offsety + lineLength, img.rows - offsety));
      ...
      case MyRECTANGLE:
        ...
        rc = Rect(center - Point(width, height) / 2,
      ...
      case MyELLIPSE:
        ...
        ellipse(img, center, Size(width / 2, height / 2), angle, 0, 360,
                Scalar(i), -1);
        break;
    }
  }
...
int main(int argc, char **argv) {
  ...
  namedWindow(winName);
  createTrackbar("colormap", winName, &p.iColormap, 1, TrackColorMap,
                 (void *)&p);
