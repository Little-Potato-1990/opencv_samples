...
int main(int /* argc */, char** argv) {
  ...
  const int DELAY = 5;
  int lineType =
      LINE_AA;  // change it to LINE_8 to see non-antialiased graphics
  int i, width = 1000, height = 700;
  int x1 = -width / 2, x2 = width * 3 / 2, y1 = -height / 2,
      y2 = height * 3 / 2;
  RNG rng(0xFFFFFFFF);
  ...
  for (i = 0; i < NUMBER * 2; i++) {
    ...
    if (arrowed < 3)
      ...
      arrowedLine(image, pt1, pt2, randomColor(rng), rng.uniform(1, 10),
                  lineType);
