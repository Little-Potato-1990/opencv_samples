...
int main(int argc, char* argv[]) {
  const string keys =
      ...
      "{i in     |        | input image path (also switches to image detection "
      "mode) }"
  ...
  cmd_parser.about(
      "This program detects the QR-codes from camera or images using the "
      "OpenCV library.");
  ...
  g_saveDetections = cmd_parser.has("save_detections") &&
                     cmd_parser.get<bool>("save_detections");
...
static void drawQRCodeContour(Mat& color_image, const vector<Point>& corners) {
  if (!corners.empty()) {
    ...
    drawContours(color_image, contours, 0, Scalar(211, 0, 148),
...
static void drawFPS(Mat& color_image, double fps) {
  ...
  putText(color_image, convert.str(), Point(25, 25), FONT_HERSHEY_DUPLEX, 1,
          Scalar(0, 0, 255), 2);
}

static void drawQRCodeResults(Mat& frame, const vector<Point>& corners,
                              const vector<cv::String>& decode_info,
                              double fps) {
  if (!corners.empty()) {
...
static void runQR(QRCodeDetector& qrcode, const Mat& input,
  ...
  if (!g_modeMultiQR) {
    ...
    if (!g_detectOnly) {
      bool result_detection =
          qrcode.detectAndDecodeMulti(input, decode_info, corners);
...
static double processQRCodeDetection(QRCodeDetector& qrcode, const Mat& input,
                                     Mat& result, vector<Point>& corners) {
  if (input.channels() == 1)
    cvtColor(input, result, COLOR_GRAY2BGR);
  else
    input.copyTo(result);

  cout << "Run " << getQRModeString() << " on image: " << input.size() << " ("
...
int liveQRCodeDetect() {
  ...
  cout << "Press 'm' to switch between detectAndDecode and detectAndDecodeMulti"
       << endl;
  ...
  for (;;) {
    ...
    if (!result.empty()) imshow("QR code", result);

    int code = waitKey(1);
    if (code < 0 && !forceSave) continue;  // timeout
    char c = (char)code;
    if (c == ' ' || forceSave) {
      string fsuffix = cv::format("-%05d", g_save_idx++);

      string fname_input = g_out_file_name + fsuffix + "_input.png";
      cout << "Saving QR code detection input: '" << fname_input << "' ..."
           << endl;
    ...
    if (c == 'm') {
      ...
      cout << "Switching QR code mode ==> "
           << (g_modeMultiQR ? "detectAndDecodeMulti" : "detectAndDecode")
           << endl;
    }
    if (c == 'd') {
      g_detectOnly = !g_detectOnly;
      cout << "Switching QR decoder mode ==> "
           << (g_detectOnly ? "detect" : "decode") << endl;
    }
    if (c == 27) {
