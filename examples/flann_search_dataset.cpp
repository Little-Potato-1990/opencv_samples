...
// Naive program to search a query picture in a dataset illustrating usage of
// FLANN
...
int main(int argc, char* argv[]) {
  ...
  cout << "Missing or unknown defined descriptor. "
          "Only SIFT and ORB are currently interfaced here"
       << endl;
  return -1;
#endif
  ...
  std::vector<unsigned int>
      db_images_indice_range;  // store the range of indices per image
  std::vector<int>
      db_indice_2_image_lut;  // match descriptor indice to its image

  db_images_indice_range.push_back(0);
  std::vector<cv::String> files;
  utils::fs::glob(db_path, cv::String(), files);
  for (std::vector<cv::String>::iterator itr = files.begin();
       itr != files.end(); ++itr) {
  ...
  for (int i = 0; i < nbr_of_imgs; ++i) {
    ...
    std::fill(db_indice_2_image_lut.begin() + first_indice,
              db_indice_2_image_lut.begin() + last_indice, i);
  }
  ...
  if (load_db_path != String())
    index = cv::makePtr<flann::GenericIndex<cvflann::L2<float> > >(
        db_descriptors, cvflann::SavedIndexParams(load_db_path));
  else
    index = cv::makePtr<flann::GenericIndex<cvflann::L2<float> > >(
        db_descriptors, cvflann::KDTreeIndexParams(4));
...
#else
  cout << "Descriptor not listed. Set the proper FLANN distance for this "
          "descriptor"
       << endl;
  return -1;
#endif
  if (save_db_path != String()) index->save(save_db_path);

  // Return if no query image was set
  if (img_path == String()) return 0;
  ...
  //-- Step 3: retrieve the descriptors in the dataset matching the ones of the
  //query image
  // /!\ knnSearch doesn't follow OpenCV standards by not initialising empty Mat
  // properties
  const int knn = 2;
...
#define DIST_TYPE int
...
#endif
  index->knnSearch(img_descriptors, indices, dists, knn,
  ...
  for (int i = 0; i < nbr_of_imgs; ++i) {
    ...
    const unsigned int nbr_of_kpts =
        db_images_indice_range[i + 1] - db_images_indice_range[i];
    ...
    images_infos.insert(
        std::pair<float, img_info>(inverse_proportion_of_retrieved_kpts, info));
  }
  ...
  ++it;
  while ((it != images_infos.end()) &&
         (it->first < 1.1 * best_matches_proportion)) {
  ...
  //-- Step 5: filter goodmatches that belong to the best image match of the
  //dataset
  std::vector<DMatch> filtered_good_matches;
  for (std::vector<DMatch>::iterator itr(good_matches.begin());
       itr != good_matches.end(); ++itr) {
  ...
  //-- Draw matches
  Mat img_matches;
  drawMatches(img, img_keypoints, db_img, db_keypoints, filtered_good_matches,
