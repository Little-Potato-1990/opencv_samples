...
static void find_decision_boundary_NBC() {
  // learn classifier
  Ptr<NormalBayesClassifier> normalBayesClassifier =
      StatModel::train<NormalBayesClassifier>(prepare_train_data());
...
static void find_decision_boundary_SVM(double C) {
  ...
  for (int i = 0; i < sv.rows; i++) {
    ...
    circle(imgDst,
           Point(saturate_cast<int>(supportVector[0]),
                 saturate_cast<int>(supportVector[1])),
...
static void find_decision_boundary_GBT() {
  ...
  Ptr<GBTrees> gbtrees =
      StatModel::train<GBTrees>(prepare_train_data(), params);
...
static void find_decision_boundary_ANN(const Mat& layer_sizes) {
  Mat trainClasses =
      Mat::zeros((int)trainedPoints.size(), (int)classColors.size(), CV_32FC1);
...
static void find_decision_boundary_EM() {
  ...
  for (i = 0; i < nmodels; i++) {
    ...
    for (j = 0; j < samples.rows; j++) {
      if (trainedPointsMarkers[j] == i) modelSamples.push_back(samples.row(j));
    }
    ...
    for (int x = 0; x < img.cols; x += testStep) {
      ...
      for (i = 0; i < nmodels; i++) {
        if (!em_models[i].empty())
          logLikelihoods.at<double>(i) =
              em_models[i]->predict2(testSample, noArray())[0];
      }
      Point maxLoc;
...
int main() {
  cout << "Use:" << endl
       ...
       << "  key 'i' - to init (clear) the data." << endl
       << endl;
