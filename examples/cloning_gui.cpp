...
* Step 1:
* -> In the source image, select the region of interest by left click mouse
button. A Polygon ROI will be created by left clicking mouse button.
...
* Step 2:
* -> In the destination image, select the point where you want to place the ROI
in the image by left clicking mouse button.
...
void source(int event, int x, int y, int, void*) {
  if (event == EVENT_LBUTTONDOWN && !drag) {
    if (flag1 == 0) {
      ...
      var++;
      drag = 1;
      if (var > 1) line(img1, pts[var - 2], point, Scalar(0, 0, 255), 2, 8, 0);
  ...
  if (event == EVENT_MBUTTONDOWN) {
    ...
    maxy = INT_MIN;
    imshow("Source", img0);
    if (num == 1 || num == 2 || num == 3) imshow("Destination", img2);
    drag = 0;
  }
}
...
void destination(int event, int x, int y, int, void*) {
  ...
  if (event == EVENT_RBUTTONUP) {
    ...
    if (maxxd > im1.size().width || maxyd > im1.size().height || minxd < 0 ||
        minyd < 0) {
    ...
    for (int i = miny, k = minyd; i < (miny + leny); i++, k++)
      for (int j = minx, l = minxd; j < (minx + lenx); j++, l++) {
        for (int c = 0; c < channel; c++) {
          final1.at<uchar>(k, l * channel + c) =
              final.at<uchar>(i, j * channel + c);
        }
      }
...
int main() {
  ...
  cout << "---------------" << endl;
  cout << "Step 1:" << endl;
  cout << " -> In the source image, select the region of interest by left "
          "click mouse button. A Polygon ROI will be created by left clicking "
          "mouse button."
       << endl;
  cout << " -> To set the Polygon ROI, click the right mouse button or use 'd' "
          "key"
       << endl;
  cout << " -> To reset the region selected, click the middle mouse button or "
          "use 'r' key."
       << endl;

  cout << "Step 2:" << endl;
  cout << " -> In the destination image, select the point where you want to "
          "place the ROI in the image by left clicking mouse button."
       << endl;
  cout << " -> To get the cloned result, click the right mouse button or use "
          "'c' key."
       << endl;
