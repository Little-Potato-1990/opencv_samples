...
static void help(char** argv) {
  printf(
      "\nThis is a camera calibration sample that calibrates 3 horizontally "
      "placed cameras together.\n"
      "Usage: %s\n"
      ...
      "(1 by default)\n"
      "     [-o=<out_camera_params>] # the output filename for intrinsic [and "
      "extrinsic] parameters\n"
      "     [-zt]                    # assume zero tangential distortion\n"
      "     [-a=<aspectRatio>]      # fix aspect ratio (fx/fy)\n"
      "     [-p]                     # fix the principal point at the center\n"
      "     [input_data]             # input data - text file with a list of "
...
static bool run3Calibration(vector<vector<Point2f> > imagePoints1,
                            ...
                            float aspectRatio, int flags, Mat& cameraMatrix1,
                            Mat& distCoeffs1, Mat& cameraMatrix2,
                            Mat& distCoeffs2, Mat& cameraMatrix3,
                            Mat& distCoeffs3, Mat& R12, Mat& T12, Mat& R13,
                            Mat& T13) {
  int c, i;
  ...
  for (c = 1; c <= 3; c++) {
    const vector<vector<Point2f> >& imgpt0 = c == 1   ? imagePoints1
                                             : c == 2 ? imagePoints2
                                                      : imagePoints3;
    imgpt.clear();
    int N = 0;
    ...
    double err = calibrateCamera(
        objpt, imgpt, imageSize, cameraMatrix, distCoeffs, rvecs, tvecs,
  ...
  for (c = 2; c <= 3; c++) {
    const vector<vector<Point2f> >& imgpt0 =
        c == 2 ? imagePoints2 : imagePoints3;
    ...
    Mat R, T, E, F;
    double err = stereoCalibrate(objpt, imgpt, imgpt_right, cameraMatrix1,
                                 distCoeffs1, cameraMatrix, distCoeffs,
...
int main(int argc, char** argv) {
  ...
  if (parser.has("a")) flags |= CALIB_FIX_ASPECT_RATIO;
  ...
  if (inputFilename.empty() || !readStringList(inputFilename, imageList) ||
      imageList.size() == 0 || imageList.size() % 3 != 0) {
    printf(
        "Error: the input image list is not specified, or can not be read, or "
        "the number of files is not divisible by 3\n");
    return -1;
  }
  ...
  for (k = 0; k < 3; k++) imgpt[k].resize(imageList.size() / 3);
  ...
  for (i = 0; i < (int)(imageList.size() / 3); i++) {
    for (k = 0; k < 3; k++) {
      ...
      if (!view.empty()) {
        ...
        bool found = findChessboardCorners(view, boardSize, ptvec,
  ...
  run3Calibration(imgpt[0], imgpt[1], imgpt[2], imageSize, boardSize,
                  squareSize, aspectRatio, flags | CALIB_FIX_K4 | CALIB_FIX_K5,
                  cameraMatrix[0], distCoeffs[0], cameraMatrix[1],
                  distCoeffs[1], cameraMatrix[2], distCoeffs[2], R12, T12, R13,
                  T13);
  ...
  double ratio = rectify3Collinear(
      cameraMatrix[0], distCoeffs[0], cameraMatrix[1], distCoeffs[1],
      cameraMatrix[2], distCoeffs[2], imgpt[0], imgpt[2], imageSize, R12, T12,
      R13, T13, R[0], R[1], R[2], P[0], P[1], P[2], Q, -1., imageSize, 0, 0,
  ...
  for (k = 0; k < 3; k++)
    initUndistortRectifyMap(cameraMatrix[k], distCoeffs[k], R[k], P[k],
                            imageSize, CV_16SC2, map1[k], map2[k]);
  ...
  for (i = 0; i < (int)(imageList.size() / 3); i++) {
    ...
    for (k = 0; k < 3; k++) {
      ...
      Mat rview =
          canvas.colRange(k2 * imageSize.width, (k2 + 1) * imageSize.width);
      remap(view, rview, map1[k1], map2[k1], INTER_LINEAR);
    }
    printf("%s %s %s\n", imageList[i * 3].c_str(), imageList[i * 3 + 1].c_str(),
           imageList[i * 3 + 2].c_str());
    resize(canvas, small_canvas, Size(1500, 1500 / 3), 0, 0,
           INTER_LINEAR_EXACT);
    for (k = 0; k < small_canvas.rows; k += 16)
      line(small_canvas, Point(0, k), Point(small_canvas.cols, k),
           Scalar(0, 255, 0), 1);
