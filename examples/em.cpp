...
int main(int /*argc*/, char** /*argv*/) {
  ...
  const Scalar colors[] = {Scalar(0, 0, 255), Scalar(0, 255, 0),
  ...
  for (i = 0; i < N; i++) {
    ...
    Mat samples_part =
        samples.rowRange(i * nsamples / N, (i + 1) * nsamples / N);
  ...
  for (i = 0; i < nsamples; i++) {
    Point pt(cvRound(samples.at<float>(i, 0)),
             cvRound(samples.at<float>(i, 1)));
