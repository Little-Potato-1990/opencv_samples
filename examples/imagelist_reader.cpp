...
static void help(char** av) {
  cout << "\nThis program gets you started being able to read images from a "
          "list in a file\n"
          "Usage:\n./"
       << av[0] << " image_list.yaml\n"
       << "\tThis is a starter sample, to get you up and going in a copy pasta "
          "fashion.\n"
       << "\tThe program reads in an list of images from a yaml or xml file "
          "and displays\n"
       << "one at a time\n"
...
static int process(const vector<string>& images) {
  ...
  for (size_t i = 0; i < images.size(); i++) {
    Mat image =
        imread(images[i], IMREAD_GRAYSCALE);  // do grayscale processing?
    imshow("image", image);
