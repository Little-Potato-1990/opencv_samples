...
static void help(char** argv) {
  printf(
      ...
      "(or Boosting classifier, or MLP, or Knearest, or Nbayes, or Support "
      "Vector Machines - see main()) using the provided dataset.\n"
      "\n"
  ...
  printf(
      ...
      "  [-boost|-mlp|-knearest|-nbayes|-svm] # to use boost/mlp/knearest/SVM "
      "classifier instead of default Random Trees\n",
      argv[0]);
}
...
static Ptr<TrainData> prepare_train_data(const Mat& data, const Mat& responses,
                                         int ntrain_samples) {
  ...
  return TrainData::create(data, ROW_SAMPLE, responses, noArray(), sample_idx,
                           noArray(), var_type);
}

inline TermCriteria TC(int iters, double eps) {
  return TermCriteria(
      TermCriteria::MAX_ITER + (eps > 0 ? TermCriteria::EPS : 0), iters, eps);
}
...
static bool build_boost_classifier(const string& data_filename,
                                   const string& filename_to_save,
                                   const string& filename_to_load) {
  ...
  if (!filename_to_load.empty()) {
    ...
    Ptr<TrainData> tdata =
        TrainData::create(new_data, ROW_SAMPLE, new_responses, noArray(),
                          noArray(), noArray(), var_type);
  ...
  for (i = 0; i < nsamples_all; i++) {
    ...
    double r =
        std::abs(best_class - responses.at<int>(i)) < FLT_EPSILON ? 1 : 0;
    if (i < ntrain_samples)
      train_hr += r;
    else
      test_hr += r;
  ...
  if (!filename_to_save.empty()) model->save(filename_to_save);
...
static bool build_mlp_classifier(const string& data_filename,
  ...
  if (!filename_to_load.empty()) {
    ...
    Ptr<TrainData> tdata =
        TrainData::create(train_data, ROW_SAMPLE, train_responses);
  ...
  test_and_save_classifier(model, data, responses, ntrain_samples, 'A',
                           filename_to_save);
  return true;
}
...
int main(int argc, char* argv[]) {
  ...
  if (parser.has("save")) filename_to_save = parser.get<string>("save");
  if (parser.has("load"))
  ...
  if ((method == 0   ? build_rtrees_classifier(data_filename, filename_to_save,
                                             filename_to_load)
       : method == 1 ? build_boost_classifier(data_filename, filename_to_save,
                                              filename_to_load)
       : method == 2 ? build_mlp_classifier(data_filename, filename_to_save,
                                            filename_to_load)
       : method == 3 ? build_knearest_classifier(data_filename, 10)
       : method == 4 ? build_nbayes_classifier(data_filename)
       : method == 5 ? build_svm_classifier(data_filename, filename_to_save,
                                            filename_to_load)
                     : -1) < 0)
