...
struct Pix {
  ...
  bool operator>(const Pix& b) const { return cost > b.cost; }
};
...
static float local_cost(const Point& p, const Point& q,
                        const Mat& gradient_magnitude, const Mat& Iy,
                        const Mat& Ix, const Mat& zero_crossing) {
  ...
  return WEIGHT_LAP_ZERO_CROSS * zero_crossing.at<uchar>(q) +
         WEIGHT_GRADIENT_DIRECTION * (acosf(dp) + acosf(dq)) /
    ...
    if (removed.at<uchar>(p) == 0) {
      ...
      for (int i = -1; i <= 1; i++) {
        for (int j = -1; j <= 1; j++) {
          int tx = p.x + i;
          int ty = p.y + j;
          if (tx < 0 || tx >= img.cols || ty < 0 || ty >= img.rows) continue;
          if (expand.at<uchar>(ty, tx) == 0) {
            Point q = Point(tx, ty);
            float cost = cost_map.at<float>(p) +
                         local_cost(p, q, param->gradient_magnitude, param->Iy,
                                    param->Ix, param->zero_crossing);
...
static void onMouse(int event, int x, int y, int, void* userdata) {
  ...
  if (event == EVENT_LBUTTONDOWN) {
    ...
    drawContours(img_pre_render, contours,
                 static_cast<int>(contours.size()) - 1, Scalar(0, 255, 0),
                 FILLED);
    ...
    while (val_point != end) {
      ...
      Point cur = Point(param->hit_map_x.at<int>(val_point),
                        param->hit_map_y.at<int>(val_point));
...
int main(int argc, const char** argv) {
  ...
  parser.about(
      "\nThis program demonstrates implementation of 'Intelligent Scissors' "
      "algorithm designed\n"
      "by Eric N. Mortensen and William A. Barrett, and described in article\n"
      "'Intelligent Scissors for Image Composition':\n"
      "http://citeseerx.ist.psu.edu/viewdoc/"
      "download?doi=10.1.1.138.3811&rep=rep1&type=pdf\n"
      "To start drawing a new contour select a pixel, click LEFT mouse "
      "button.\n"
  ...
  param.gradient_magnitude.convertTo(param.gradient_magnitude, CV_32F,
                                     -1 / max_val, 1.0);
