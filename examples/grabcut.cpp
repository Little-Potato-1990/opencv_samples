...
static void help(char** argv) {
  cout << "\nThis program demonstrates GrabCut segmentation -- select an "
          "object in a region\n"
...
void GCApplication::showImage() const {
  ...
  if (rectState == IN_PROCESS || rectState == SET)
    rectangle(res, Point(rect.x, rect.y),
...
void GCApplication::mouseClick(int event, int x, int y, int flags, void*) {
  ...
  switch (event) {
    case EVENT_LBUTTONDOWN:  // set rect or GC_BGD(GC_FGD) labels
    {
      ...
      if ((isb || isf) && rectState == SET) lblsState = IN_PROCESS;
    } break;
    case EVENT_RBUTTONDOWN:  // set GC_PR_BGD(GC_PR_FGD) labels
    {
      bool isb = (flags & BGD_KEY) != 0, isf = (flags & FGD_KEY) != 0;
      if ((isb || isf) && rectState == SET) prLblsState = IN_PROCESS;
    } break;
