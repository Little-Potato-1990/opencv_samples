...
inline Ptr<VideoCapture> createCapture(const string &backend,
...
inline Ptr<VideoWriter> createWriter(const string &backend,
                                     const string &file_name,
                                     const string &codec, Size sz,
                                     unsigned fps) {
  if (backend == "gst-default") {
    cout << "Created GStreamer writer ( " << file_name << ", FPS=" << fps
         << ", Size=" << sz << ")" << endl;
    return makePtr<VideoWriter>(
        file_name, CAP_GSTREAMER,
    ...
    cout << "Created FFmpeg writer ( " << file_name << ", FPS=" << fps
         << ", Size=" << sz << " )" << endl;
    return makePtr<VideoWriter>(
        file_name, CAP_FFMPEG,
        getValue(fourccByCodec(), codec, "Invalid codec"), fps, sz, true);
  }
  return Ptr<VideoWriter>();
}
...
int main(int argc, char *argv[]) {
  const string keys =
      ...
      "{b backend      |default    | video backend (supported: 'gst-default', "
      "'gst-basic', 'gst-vaapi', 'gst-libav', 'gst-mfx', 'ffmpeg') }"
      "{c codec        |h264       | codec name     (supported: 'h264', "
      "'h265', 'mpeg2', 'mpeg4', 'mjpeg', 'vp8') }"
      "{f file path    |           | path to file }"
      "{r resolution   |720p       | video resolution for encoding (supported: "
      "'720p', '1080p', '4k') }"
      "{fps            |30         | fix frame per second for encoding "
      "(supported: fps > 0) }"
      "{fast           |           | fast measure fps }";
  CommandLineParser cmd_parser(argc, argv, keys);
  cmd_parser.about(
      "This program measures performance of video encoding and decoding using "
  ...
  cout << "Mode: " << mode << ", Backend: " << backend
       << ", File: " << file_name << ", Codec: " << codec << endl;
  ...
  try {
    if (mode == "decode") {
      ...
      if (!cap || !wrt) {
        cout << "Failed to create synthetic video source or video writer"
             << endl;
        return -3;
      }
  ...
  if (tick.getCounter() == 0) {
    ...
    cout << tick.getCounter() << " frames in " << tick.getTimeSec() << " sec ~ "
         << res_fps << " FPS"
         << " (total time: " << total.getTimeSec() << " sec)" << endl;
  }
  return 0;
}
