...
static void help(char** argv) {
  cout << "\n"
          ...
          "   cell\n"
          " - Transform histograms to space with Hellinger metric (see [1] "
          "(RootSIFT))\n"
          "\n"
          "\n"
          "[1] R. Arandjelovic, A. Zisserman\n"
          "    \"Three things everyone should know to improve object "
          "retrieval\"\n"
          "    "
          "http://www.robots.ox.ac.uk/~vgg/publications/2012/Arandjelovic12/"
          "arandjelovic12.pdf\n"
          "\n"
          "Usage:\n"
       << argv[0] << endl;
...
static void split2d(const Mat& image, const Size cell_size,
                    vector<Mat>& cells) {
...
static void load_digits(const char* fn, vector<Mat>& digits,
                        vector<int>& labels) {
  digits.clear();
  labels.clear();
...
static void deskew(const Mat& img, Mat& deskewed_img) {
  ...
  warpAffine(img, deskewed_img, M, Size(SZ, SZ),
...
static void mosaic(const int width, const vector<Mat>& images, Mat& grid) {
  ...
  if (!images.empty()) {
    ...
    for (size_t i = 0; i < images.size(); i++) {
      Mat location_on_grid =
          grid(Rect(SZ * ((int)i % width), SZ * ((int)i / width), SZ, SZ));
      images[i].copyTo(location_on_grid);
    }
  }
}

static void evaluate_model(const vector<float>& predictions,
                           const vector<Mat>& digits, const vector<int>& labels,
                           Mat& mos) {
  double err = 0;
...
static void bincount(const Mat& x, const Mat& weights, const int min_length,
                     vector<double>& bins) {
  double max_x_val = 0;
...
static void preprocess_hog(const vector<Mat>& digits, Mat& hog) {
  ...
  for (size_t img_index = 0; img_index < digits.size(); img_index++) {
    ...
    Mat bin_cells[] = {bin(Rect(0, 0, half_cell, half_cell)),
    ...
    Mat mag_cells[] = {mag(Rect(0, 0, half_cell, half_cell)),
    ...
    for (size_t i = 0; i < hist.size(); i++) {
      hog.at<float>((int)img_index, (int)i) =
          (float)(hist[i] / (hist_norm + eps));
    }
  }
}
...
int main(int /* argc */, char* argv[]) {
  ...
  Mat samples_test =
      samples(Rect(0, train_n, samples.cols, samples.rows - train_n));
