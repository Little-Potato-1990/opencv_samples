//  This example provides a digital recognition based on LeNet-5 and connected
//  component analysis. It makes it possible for OpenCV beginner to run dnn
//  models in real time using only CPU. It can read pictures from the camera in
//  real time to make predictions, and display the recognized digits as overlays
//  on top of the original digits.
//
//  In order to achieve a better display effect, please write the number on
//  white paper and occupy the entire camera.
//
//  You can follow the following guide to train LeNet-5 by yourself using the
//  MNIST dataset.
...
const char *keys =
    ...
    "{ modelBin    |     | Path to a binary .caffemodel file contains trained "
    "network.}"
    "{ modelTxt    |     | Path to a .prototxt file contains the model "
    "definition of trained network.}"
...
int main(int argc, char **argv) {
  ...
  const std::string resultWinName =
      "Please write the number on white paper and occupy the entire camera.";
  ...
  while (waitKey(1) < 0) {
    ...
    for (int i = 1; i < nccomps; i++) {
      ...
      if (stats.at<int>(i - 1, CC_STAT_AREA) > 80 &&
          stats.at<int>(i - 1, CC_STAT_AREA) < 3000) {
        ifDrawingBox = true;
        int left = stats.at<int>(i - 1, CC_STAT_HEIGHT) / 4;
        getRectangle = Rect(stats.at<int>(i - 1, CC_STAT_LEFT) - left,
                            stats.at<int>(i - 1, CC_STAT_TOP) - left,
                            stats.at<int>(i - 1, CC_STAT_WIDTH) + 2 * left,
                            stats.at<int>(i - 1, CC_STAT_HEIGHT) + 2 * left);
      ...
      if (ifDrawingBox && !getRectangle.empty()) {
        ...
        putText(rawImage, std::to_string(classId), position, 3, 1.0,
    ...
    putText(rawImage, fpsString, Point(5, 20), FONT_HERSHEY_SIMPLEX, 0.6,
            Scalar(128, 255, 128));
