...
static void detectFacialFeaures(Mat&, const vector<Rect_<int> >, string, string,
                                string);
...
int main(int argc, char** argv) {
  cv::CommandLineParser parser(
      argc, argv, "{eyes||}{nose||}{mouth||}{help h||}{@image||}{@facexml||}");
  if (parser.has("help")) {
    help(argv);
    return 0;
  }
  ...
  detectFacialFeaures(image, faces, eye_cascade_path, nose_cascade_path,
                      mouth_cascade_path);
...
static void help(char** argv) {
  cout << "\nThis file demonstrates facial feature points detection using "
          "Haarcascade classifiers.\n"
          "The program detects a face and eyes, nose and mouth inside the face."
          "The code has been tested on the Japanese Female Facial Expression "
          "(JAFFE) database and found"
          "to give reasonably accurate results. \n";

  cout << "\nUSAGE: " << argv[0]
       << " [IMAGE] [FACE_CASCADE] [OPTIONS]\n"
          "IMAGE\n\tPath to the image of a face taken as input.\n"
          "FACE_CASCSDE\n\t Path to a haarcascade classifier for face "
          "detection.\n"
          "OPTIONS: \nThere are 3 options available which are described in "
          "detail. There must be a "
          "space between the option and it's argument (All three options "
          "accept arguments).\n"
          "\t-eyes=<eyes_cascade> : Specify the haarcascade classifier for eye "
          "detection.\n"
          "\t-nose=<nose_cascade> : Specify the haarcascade classifier for "
          "nose detection.\n"
          "\t-mouth=<mouth-cascade> : Specify the haarcascade classifier for "
          "mouth detection.\n";
...
static void detectFaces(Mat& img, vector<Rect_<int> >& faces,
  ...
  if (!face_cascade.empty())
    face_cascade.detectMultiScale(img, faces, 1.15, 3, 0 | CASCADE_SCALE_IMAGE,
                                  Size(30, 30));
  return;
}

static void detectFacialFeaures(Mat& img, const vector<Rect_<int> > faces,
                                string eye_cascade, string nose_cascade,
  ...
  for (unsigned int i = 0; i < faces.size(); ++i) {
    ...
    // Eyes, nose and mouth will be detected inside the face (region of
    // interest)
    ...
    if (!eye_cascade.empty()) {
      ...
      for (unsigned int j = 0; j < eyes.size(); ++j) {
        Rect e = eyes[j];
        circle(ROI, Point(e.x + e.width / 2, e.y + e.height / 2), 3,
               Scalar(0, 255, 0), -1, 8);
    ...
    if (!nose_cascade.empty()) {
      ...
      for (unsigned int j = 0; j < nose.size(); ++j) {
        Rect n = nose[j];
        circle(ROI, Point(n.x + n.width / 2, n.y + n.height / 2), 3,
               Scalar(0, 255, 0), -1, 8);
    ...
    if (!mouth_cascade.empty()) {
      ...
      for (unsigned int j = 0; j < mouth.size(); ++j) {
        ...
        if ((is_full_detection) && (mouth_center_height > nose_center_height)) {
          rectangle(ROI, Point(m.x, m.y), Point(m.x + m.width, m.y + m.height),
                    Scalar(0, 255, 0), 1, 4);
...
static void detectEyes(Mat& img, vector<Rect_<int> >& eyes,
  ...
  if (!eyes_cascade.empty())
    eyes_cascade.detectMultiScale(img, eyes, 1.20, 5, 0 | CASCADE_SCALE_IMAGE,
                                  Size(30, 30));
  return;
}
...
static void detectNose(Mat& img, vector<Rect_<int> >& nose,
  ...
  if (!nose_cascade.empty())
    nose_cascade.detectMultiScale(img, nose, 1.20, 5, 0 | CASCADE_SCALE_IMAGE,
                                  Size(30, 30));
  return;
}
...
static void detectMouth(Mat& img, vector<Rect_<int> >& mouth,
  ...
  if (!mouth_cascade.empty())
    mouth_cascade.detectMultiScale(img, mouth, 1.20, 5, 0 | CASCADE_SCALE_IMAGE,
                                   Size(30, 30));
  return;
}
