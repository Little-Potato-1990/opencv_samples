 ...
 * the first image is the input image and the second one defines the template
 * image. In the latter case, you can also parse the warp's initialization.
 *
...
static void draw_warped_roi(Mat& image, const int width, const int height,
                            Mat& W);
...
const std::string keys =
    ...
    "{m motionType   | affine        | type of motion (translation, euclidean, "
    "affine, homography) }"
...
static void help(const char** argv) {
  cout << "\nThis file demonstrates the use of the ECC image alignment "
          "algorithm. When one image"
          " is given, the template image is artificially formed by a random "
          "warp. When both images"
          " are given, the initialization of the warp by command line parsing "
          "is possible. "
          "If inputWarp is missing, the identity transformation initializes "
          "the algorithm. \n"
       << endl;
...
int main(const int argc, const char* argv[]) {
  ...
  if (tempImgFile != "") {
    ...
    switch (mode_temp) {
      case MOTION_TRANSLATION:
        warpGround = (Mat_<float>(2, 3) << 1, 0, (rng.uniform(10.f, 20.f)), 0,
                      1, (rng.uniform(10.f, 20.f)));
        warpAffine(target_image, template_image, warpGround, Size(200, 200),
                   INTER_LINEAR + WARP_INVERSE_MAP);
        break;
      case MOTION_EUCLIDEAN:
        angle =
            CV_PI / 30 + CV_PI * rng.uniform((double)-2.f, (double)2.f) / 180;

        warpGround = (Mat_<float>(2, 3) << cos(angle), -sin(angle),
                      (rng.uniform(10.f, 20.f)), sin(angle), cos(angle),
      ...
      case MOTION_HOMOGRAPHY:
        warpGround =
            (Mat_<float>(3, 3) << (1 - rng.uniform(-0.05f, 0.05f)),
             (rng.uniform(-0.03f, 0.03f)), (rng.uniform(10.f, 20.f)),
             (rng.uniform(-0.03f, 0.03f)), (1 - rng.uniform(-0.05f, 0.05f)),
             (rng.uniform(10.f, 20.f)), (rng.uniform(0.0001f, 0.0003f)),
             (rng.uniform(0.0001f, 0.0003f)), 1.f);
  ...
  if (warp_mode != MOTION_HOMOGRAPHY) warp_matrix.rows = 2;
  ...
  if (cc == -1) {
    cerr << "The execution was interrupted. The correlation value is going to "
            "be minimized."
         << endl;
  ...
  if (verbose) {
    cout << "Alignment time (" << warpType << " transformation): " << total_time
         << " sec" << endl
         << flush;
  ...
  if (verbose) {
    cout << "\nThe final warp has been saved in the file: " << finalWarp << endl
         << flush;
  }
  ...
  if (warp_mode != MOTION_HOMOGRAPHY)
    ...
    warpPerspective(target_image, warped_image, warp_matrix,
                    warped_image.size(), INTER_LINEAR + WARP_INVERSE_MAP);
  ...
  if (verbose) {
    cout << "The warped image has been saved in the file: " << warpedImFile
         << endl
         << flush;
    ...
    draw_warped_roi(target_image, template_image.cols - 2,
                    template_image.rows - 2, warp_matrix);
    draw_warped_roi(template_image, template_image.cols - 2,
                    template_image.rows - 2, identity_matrix);
    ...
    // show images
    cout << "Press any key to exit the demo (you might need to click on the "
            "images before)."
         << endl
         << flush;
