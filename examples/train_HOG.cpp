...
void sample_neg(const vector<Mat>& full_neg_lst, vector<Mat>& neg_lst,
                const Size& size);
void computeHOGs(const Size wsize, const vector<Mat>& img_lst,
...
int main(int argc, char** argv) {
  const char* keys = {
      ...
      "{f     |false| indicates if the program will generate and use mirrored "
      "samples or not}"
      "{d     |false| train twice}"
  ...
  if (pos_dir.empty() || neg_dir.empty()) {
    ...
    cout << "Wrong number of parameters.\n\n"
         ...
         << argv[0]
         << " -dw=64 -dh=128 -pd=/INRIAPerson/96X160H96/Train/pos "
            "-nd=/INRIAPerson/neg -td=/INRIAPerson/Test/pos "
            "-fn=HOGpedestrian64x128.xml -d\n"
  ...
  clog << "...[done] ( positive images count : " << positive_count << " )"
       << endl;
  ...
  clog << "...[done] ( negative images count : " << negative_count << " )"
       << endl;
  ...
  if (train_twice) {
    clog << "Testing trained detector on negative images. This might take a "
            "few minutes...";
