...
static void help(char** argv) {
  printf(
      ...
      "     [-d=<delay>]             # a minimum delay in ms between "
      "subsequent attempts to capture a next view\n"
      ...
      "horizontal axis\n"
      "     [-V]                     # use a video file, and not an image "
      "list, uses\n"
      "                              # [input_data] string for the video file "
      "name\n"
      "     [-su]                    # show undistorted images after "
      "calibration\n"
      "     [-ws=<number_of_pixel>]  # Half of search window for cornerSubPix "
      "(11 by default)\n"
      "     [-dt=<distance>]         # actual distance between top-left and "
      "top-right corners of\n"
      "                              # the calibration grid. If this parameter "
      "is specified, a more\n"
      "                              # accurate calibration method will be "
      "used which may be better\n"
      "                              # with inaccurate, roughly planar "
      "target.\n"
      ...
      "imagelist_creator\n"
      "                              #  - name of video file with a video of "
      "the board\n"
      "                              # if input_data not specified, a live "
      "view from the camera is used\n"
      "\n",
      argv[0]);
  printf("\n%s", usage);
  printf("\n%s", liveCaptureHelp);
...
static bool runCalibration(vector<vector<Point2f> > imagePoints, Size imageSize,
                           Size boardSize, Pattern patternType,
                           float squareSize, float aspectRatio,
                           float grid_width, bool release_object, int flags,
...
static bool runAndSave(const string& outputFilename,
  ...
  bool ok = runCalibration(imagePoints, imageSize, boardSize, patternType,
                           squareSize, aspectRatio, grid_width, release_object,
                           flags, cameraMatrix, distCoeffs, rvecs, tvecs,
                           reprojErrs, newObjPoints, totalAvgErr);
    ...
    if (view.empty()) {
      if (imagePoints.size() > 0)
        runAndSave(outputFilename, imagePoints, imageSize, boardSize, pattern,
                   ...
                   cameraMatrix, distCoeffs, writeExtrinsics, writePoints,
                   writeGrid);
      break;
    }
