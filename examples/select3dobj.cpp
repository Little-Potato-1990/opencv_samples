/*
 *
 * select3obj.cpp With a calibration chessboard on a table, mark an object in a
 * 3D box and track that object in all subsequent frames as long as the camera
 * can see the chessboard. Also segments the object using the box projection.
 * This program is useful for collecting large datasets of many views of an
 * object on a table.
 *
 */
...
static string helphelp(char** argv) {
  return string(
             "\nThis program's purpose is to collect data sets of an object "
             "and its segmentation mask.\n") +
         "\n"
         "It shows how to use a calibrated camera together with a calibration "
         "pattern to\n"
         "compute the homography of the plane the calibration pattern is on. "
         "It also shows grabCut\n"
         "segmentation etc.\n"
         "\n" +
         argv[0] +
         ...
         " [-s=<square_size>]            Optional measure of chessboard "
         "squares in meters\n"
         " -i=<camera_intrinsics_filename> Camera matrix .yml file from "
         "calibration.cpp\n"
         " -o=<output_prefix>        Prefix the output segmentation images "
         "with this\n"
         " [video_filename/cameraId]  If present, read from that video file or "
         "that ID\n"
         "\n"
         "Using a camera's intrinsics (from calibrating a camera -- see "
         "calibration.cpp) and an\n"
         "image of the object sitting on a planar surface with a calibration "
         "pattern of\n"
         "(board_width x board_height) on the surface, we draw a 3D box around "
         "the object. From\n"
         "then on, we can move a camera and as long as it sees the chessboard "
         "calibration pattern,\n"
         "it will store a mask of where the object is. We get successive "
         "images using <output_prefix>\n"
         "of the segmentation mask containing the object. This makes creating "
         "training sets easy.\n"
         "It is best if the chessboard is odd x even in dimensions to avoid "
         "ambiguous poses.\n"
         "\n"
         "The actions one can use while the program is running are:\n"
         "\n"
         "  Select object as 3D box with the mouse.\n"
         "   First draw one line on the plane to outline the projection of "
         "that object on the plane\n"
         "    Then extend that line into a box to encompass the projection of "
         "that object onto the plane\n"
         "    The use the mouse again to extend the box upwards from the plane "
         "to encase the object.\n"
         "  Then use the following commands\n"
         "    ESC   - Reset the selection\n"
         "    SPACE - Skip the frame; move to the next frame (not in video "
         "mode)\n"
...
static bool readCameraMatrix(const string& filename, Mat& cameraMatrix,
                             Mat& distCoeffs, Size& calibratedImageSize) {
  ...
  if (distCoeffs.type() != CV_64F) distCoeffs = Mat_<double>(distCoeffs);
  if (cameraMatrix.type() != CV_64F) cameraMatrix = Mat_<double>(cameraMatrix);
  ...
  if (!shownFrame.empty()) {
    if (nobjpt == 1)
      ...
      for (int i = 0; i < 4; i++) {
        ...
        line(shownFrame, imgpt[i], imgpt[(i + 1) % 4], Scalar(0, 255, 0), 3,
             LINE_AA);
      }
    else
      for (int i = 0; i < 8; i++) {
        circle(shownFrame, imgpt[i], 3, Scalar(0, 255, 0), -1, LINE_AA);
        line(shownFrame, imgpt[i], imgpt[(i + 1) % 4 + (i / 4) * 4],
             Scalar(0, 255, 0), 3, LINE_AA);
        line(shownFrame, imgpt[i], imgpt[i % 4], Scalar(0, 255, 0), 3, LINE_AA);
      }
  }

  if (nobjpt <= 2) return Rect();
  vector<Point> hull;
  convexHull(Mat_<Point>(Mat(imgpt)), hull);
  Mat selectedObjMask = Mat::zeros(frame.size(), CV_8U);
  fillConvexPoly(selectedObjMask, &hull[0], (int)hull.size(), Scalar::all(255),
                 8, 0);
  Rect roi = boundingRect(Mat(hull)) & Rect(Point(), frame.size());

  if (runExtraSegmentation) {
    selectedObjMask = Scalar::all(GC_BGD);
    fillConvexPoly(selectedObjMask, &hull[0], (int)hull.size(),
                   Scalar::all(GC_PR_FGD), 8, 0);
    Mat bgdModel, fgdModel;
    grabCut(frame, selectedObjMask, roi, bgdModel, fgdModel, 3,
...
static int select3DBox(const string& windowname, const string& selWinName,
                       const Mat& frame, const Mat& cameraMatrix,
  ...
  for (;;) {
    ...
    if ((mouse.event == EVENT_LBUTTONDOWN || mouse.event == EVENT_LBUTTONUP ||
         dragging) &&
        nobjpt < 4) {
      Point2f m = mouse.pt;
      ...
      box[npt] =
          image2plane(imgpt[npt], R, tvec, cameraMatrix, npt < 3 ? 0 : Z);
    ...
    extract3DBox(frame, shownFrame, selectedObjFrame, cameraMatrix, rvec, tvec,
                 box, npt, false);
    ...
    if ((c == '\r' || c == '\n') && nobjpt == 4 && box[3].z != 0) return 1;
  }
}

static bool readModelViews(const string& filename, vector<Point3f>& box,
                           vector<string>& imagelist, vector<Rect>& roiList,
  ...
  if (all.type() != FileNode::SEQ) return false;
...
int main(int argc, char** argv) {
  string help = string("Usage: ") + argv[0] +
                " -w=<board_width> -h=<board_height> [-s=<square_size>]\n" +
                "\t-i=<intrinsics_filename> -o=<output_prefix> "
                "[video_filename/cameraId]\n";
  ...
  cv::CommandLineParser parser(
      argc, argv, "{help h||}{w||}{h||}{s|1|}{i||}{o||}{@input|0|}");
  if (parser.has("help")) {
  ...
  if (parser.get<string>("@input").size() == 1 &&
      isdigit(parser.get<string>("@input")[0]))
  ...
  readCameraMatrix(intrinsicsFilename, cameraMatrix, distCoeffs,
                   calibratedImageSize);
  ...
  const char* outbarename = 0;
  {
    ...
    if (tmp && tmp > outbarename) outbarename = tmp;
    if (outbarename) {
  ...
  for (int i = 0;; i++) {
    Mat frame0;
    if (!imageList.empty()) {
      if (i < (int)imageList.size()) frame0 = imread(string(imageList[i]), 1);
    } else
      capture >> frame0;
    ...
    if (frame.empty()) {
      ...
      initUndistortRectifyMap(cameraMatrix, distCoeffs, Mat(), cameraMatrix,
                              frame0.size(), CV_32FC2, mapxy, dummy);
    ...
    frame.copyTo(shownFrame);
    drawChessboardCorners(shownFrame, boardSize, Mat(foundBoardCorners),
                          boardFound);
    selectedObjFrame = Mat::zeros(frame.size(), frame.type());

    if (boardFound && grabNext) {
      if (box.empty()) {
        int code = select3DBox("View", "Selected Object", frame, cameraMatrix,
                               rvec, tvec, box);
        if (code == -100) break;
      }

      if (!box.empty()) {
        Rect r = extract3DBox(frame, shownFrame, selectedObjFrame, cameraMatrix,
                              rvec, tvec, box, 4, true);
        if (!r.empty()) {
