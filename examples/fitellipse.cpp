 ...
 *  1: OpenCV's original method fitEllipse which implements Fitzgibbon 1995
 *method. 2: The Approximate Mean Square (AMS) method fitEllipseAMS  proposed by
 *Taubin 1991 3: The Direct least square (Direct) method fitEllipseDirect
 *proposed by Fitzgibbon1999.
 *
 *  Trackbar specify threshold parameter.
 *
 *  White lines is contours/input points and the true ellipse used to generate
 *the data. 1: Blue lines is fitting ellipses using openCV's original method. 2:
 *Green lines is fitting ellipses using the AMS method. 3: Red lines is fitting
 *ellipses using the Direct method.
 *
 *
...
class canvas {
 public:
  ...
  void stretch(vector<Point2f> pts) {  // Stretch the canvas so all the points
                                       // pts are on the canvas.
  ...
  void stretch(cv::RotatedRect box) {  // Stretch the canvas so that the
                                       // rectangle box is on the canvas.
  ...
  void drawEllipseWithBox(cv::RotatedRect box, cv::Scalar color,
                          int lineThickness) {
    if (img.empty()) {
      stretch(box);
    ...
    box.center =
        scale * cv::Point2f(box.center.x - origin.x, box.center.y - origin.y);
  ...
  void drawPoints(vector<Point2f> pts, cv::Scalar color) {
    ...
    for (size_t i = 0; i < pts.size(); i++) {
      Point2f pnt =
          scale * cv::Point2f(pts[i].x - origin.x, pts[i].y - origin.y);
  ...
  void drawLabels(std::vector<std::string> text,
...
static void help(char** argv) {
  cout << "\nThis program is demonstration for ellipse fitting. The program "
          "finds\n"
          "contours and approximate it by ellipses. Three methods are used to "
          "find the \n"
...
void processImage(int /*h*/, void*) {
  ...
  canvas paper;
  paper.init(int(0.8 * MIN(bimage.rows, bimage.cols)),
             int(1.2 * MAX(bimage.rows, bimage.cols)));
  paper.stretch(
      cv::Point2f(0.0f, 0.0f),
      cv::Point2f((float)(bimage.cols + 2.0), (float)(bimage.rows + 2.0)));
  ...
  for (size_t i = 0; i < contours.size(); i++) {
    ...
    for (int j = 0; j < pointsf.rows; j++) {
      ...
      if ((pnt.x > margin && pnt.y > margin && pnt.x < bimage.cols - margin &&
           pnt.y < bimage.rows - margin)) {
        if (j % 20 == 0) {
          pts.push_back(pnt);
        }
      }
  ...
  for (size_t i = 0; i < points.size(); i++) {
    ...
    if (fitEllipseAMSQ) {
      ...
      if (MAX(boxAMS.size.width, boxAMS.size.height) >
              MIN(boxAMS.size.width, boxAMS.size.height) * 30 ||
          MAX(box.size.width, box.size.height) <= 0 ||
          MIN(box.size.width, box.size.height) <= 0) {
        continue;
      };
    ...
    if (fitEllipseDirectQ) {
      ...
      if (MAX(boxDirect.size.width, boxDirect.size.height) >
              MIN(boxDirect.size.width, boxDirect.size.height) * 30 ||
          MAX(box.size.width, box.size.height) <= 0 ||
          MIN(box.size.width, box.size.height) <= 0) {
        continue;
      };
