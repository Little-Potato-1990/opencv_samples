...
static void help(char* argv[]) {
  cout << "\n This program demonstrates how to detect compute and match ORB "
          "BRISK and AKAZE descriptors \n"
          "Usage: \n  "
       << argv[0]
       << " --image1=<image1(basketball1.png as default)> "
          "--image2=<image2(basketball2.png as default)>\n"
...
int main(int argc, char* argv[]) {
  ...
  typeDesc.push_back(
      "AKAZE-DESCRIPTOR_KAZE_UPRIGHT");  // see
                                         // https://docs.opencv.org/master/d8/d30/classcv_1_1AKAZE.html
  typeDesc.push_back(
      "AKAZE");  // see
                 // http://docs.opencv.org/master/d8/d30/classcv_1_1AKAZE.html
  typeDesc.push_back(
      "ORB");  // see http://docs.opencv.org/master/de/dbf/classcv_1_1BRISK.html
  typeDesc.push_back(
      "BRISK");  // see http://docs.opencv.org/master/db/d95/classcv_1_1ORB.html
  // This algorithm would be used to match descriptors see
  // http://docs.opencv.org/master/db/d39/classcv_1_1DescriptorMatcher.html#ab5dc5036569ecc8d47565007fa518257
  ...
  for (itDesc = typeDesc.begin(); itDesc != typeDesc.end(); ++itDesc) {
    ...
    try {
      ...
      // Match method loop
      for (itMatcher = typeAlgoMatch.begin(); itMatcher != typeAlgoMatch.end();
           ++itMatcher) {
        descriptorMatcher = DescriptorMatcher::create(*itMatcher);
        if ((*itMatcher == "BruteForce-Hamming" ||
             *itMatcher == "BruteForce-Hamming(2)") &&
            (b->descriptorType() == CV_32F || b->defaultNorm() <= NORM_L2SQR)) {
          cout << "************************************************************"
                  "**************\n";
          cout << "It's strange. You should use Hamming distance only for a "
                  "binary descriptor\n";
          cout << "************************************************************"
                  "**************\n";
        }
        if ((*itMatcher == "BruteForce" || *itMatcher == "BruteForce-L1") &&
            (b->defaultNorm() >= NORM_HAMMING)) {
          cout << "************************************************************"
                  "**************\n";
          cout << "It's strange. You shouldn't use L1 or L2 distance for a "
                  "binary descriptor\n";
          cout << "************************************************************"
                  "**************\n";
        }
        try {
          ...
          FileStorage fs(*itDesc + "_" + *itMatcher + ".yml",
                         FileStorage::WRITE);
          fs << "Matches" << matches;
          ...
          // Use to compute distance between keyPoint matches and to evaluate
          // match algorithm
  ...
  int i = 0;
  cout << "Cumulative distance between keypoint match for different algorithm "
          "and feature detector \n\t";
  cout << "We cannot say which is the best but we can say results are "
          "different! \n\t";
