...
static float calculateAccuracyPercent(const Mat &original,
                                      const Mat &predicted) {
...
int main() {
  ...
  cout << "********************************************************************"
       ...
       << endl;
  cout << filename
       << " contains digits 0 and 1 of 20 samples each, collected on an "
          "Android device"
       << endl;
  cout << "Each of the collected images are of size 28 x 28 re-arranged to 1 x "
          "784 matrix"
       << endl;
  cout << "********************************************************************"
          "**"
  ...
  cout << "training/testing samples count: " << data_train.rows << "/"
       << data_test.rows << endl;
  ...
  cout << "accuracy: " << calculateAccuracyPercent(labels_test, responses)
       << "%" << endl;
  ...
  Ptr<LogisticRegression> lr2 =
      StatModel::load<LogisticRegression>(saveFilename);
  ...
  cout << "accuracy: " << calculateAccuracyPercent(labels_test, responses2)
       << "%" << endl;
