...
static void help(char** argv) {
  printf(
      "\nThis sample demonstrates how to use different decision trees and "
      "forests including boosting and random trees.\n"
      "Usage:\n\t%s [-r=<response_column>] [-ts=type_spec] <csv filename>\n"
      "where -r=<response_column> specified the 0-based index of the response "
      "(0 by default)\n"
      "-ts= specifies the var type spec in the form "
      "ord[n1,n2-n3,n4-n5,...]cat[m1-m2,m3,m4-m5,...]\n"
      "<csv filename> is the name of training data file in comma-separated "
      "value format\n\n",
      argv[0]);
}

static void train_and_print_errs(Ptr<StatModel> model,
...
int main(int argc, char** argv) {
  ...
  Ptr<TrainData> data = TrainData::loadFromCSV(filename, 0, response_idx,
                                               response_idx + 1, typespec);
  if (data.empty()) {
  ...
  std::cout << "Test/Train: " << data->getNTestSamples() << "/"
            << data->getNTrainSamples();
  ...
  if ((int)data->getClassLabels().total() <=
      2)  // regression or 2-class classification problem
  {
    printf("======BOOST=====\n");
