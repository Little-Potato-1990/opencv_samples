...
static void help(char** argv) {
  cout << "\n This program demonstrates how to use BLOB to detect and filter "
          "region \n"
       << "Usage: \n"
...
static String Legende(SimpleBlobDetector::Params& pAct) {
  ...
  if (pAct.filterByArea) {
    String inf =
        static_cast<const ostringstream&>(ostringstream() << pAct.minArea)
            .str();
    String sup =
        static_cast<const ostringstream&>(ostringstream() << pAct.maxArea)
            .str();
  ...
  if (pAct.filterByCircularity) {
    String inf = static_cast<const ostringstream&>(ostringstream()
                                                   << pAct.minCircularity)
                     .str();
    String sup = static_cast<const ostringstream&>(ostringstream()
                                                   << pAct.maxCircularity)
                     .str();
    if (s.length() == 0)
  ...
  if (pAct.filterByConvexity) {
    String inf =
        static_cast<const ostringstream&>(ostringstream() << pAct.minConvexity)
            .str();
    String sup =
        static_cast<const ostringstream&>(ostringstream() << pAct.maxConvexity)
            .str();
    if (s.length() == 0)
  ...
  if (pAct.filterByInertia) {
    String inf = static_cast<const ostringstream&>(ostringstream()
                                                   << pAct.minInertiaRatio)
                     .str();
    String sup = static_cast<const ostringstream&>(ostringstream()
                                                   << pAct.maxInertiaRatio)
                     .str();
    if (s.length() == 0)
...
int main(int argc, char* argv[]) {
  ...
  // These descriptors are going to be detecting and computing BLOBS with 6
  // different params Param for first BLOB detector we want all
  typeDesc.push_back(
      "BLOB");  // see
  ...
  // Param for six BLOB detector we want blob with gravity center color equal to
  // 0
  typeDesc.push_back("BLOB");
  pBLOB.push_back(pDefaultBLOB);
  ...
  for (itDesc = typeDesc.begin(); itDesc != typeDesc.end(); ++itDesc) {
    ...
    try {
      ...
      if (b.dynamicCast<SimpleBlobDetector>().get()) {
        ...
        int i = 0;
        for (vector<KeyPoint>::iterator k = keyImg.begin(); k != keyImg.end();
             ++k, ++i)
